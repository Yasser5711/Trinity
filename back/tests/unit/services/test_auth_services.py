import pytest
from db.schemas.auth_schemas import UserCreate, UserLogin, UserUpdateInfo, ResetPass
...
def test_register_user_success(db_session):
    user_data = UserCreate(
        first_name="Jane", last_name="Doe", email="janedoe@test.com", password="123456"
    )
...
def test_login_user_success(db_session, sample_user):
    creds = UserLogin(email=sample_user.email, password="password123")
...
def test_reset_password_success(db_session, sample_user):
    ...
    reset_data = ResetPass(token=token, password="newpass",
