...
from db.models.models import CartItem, Invoice, Stock
...
def test_create_and_get_cart(db_session, sample_user):
    ...
    fetched = client_cart_repository.get_cart_by_user(db_session, sample_user.id)
...
def test_get_user_invoices(db_session, sample_user, sample_cart_item):
    ...
    invoices = client_cart_repository.get_user_invoices(db_session, sample_user.id)
