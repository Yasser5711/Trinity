...
def test_create_cart_item(db_session, sample_cart, sample_product):
    item = CartItem(cart_id=sample_cart.id, product_id=sample_product.id, quantity=1)
...
def test_delete_cart_item(db_session, sample_cart_item):
    ...
    assert cart_item_repository.get_by_id(db_session, sample_cart_item.id) is None


def test_get_product_by_id_found(db_session, sample_product):
    product = cart_item_repository.get_product_by_id(db_session, sample_product.id)
