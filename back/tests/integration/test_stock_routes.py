...
def test_get_stock_by_id(client, admin_auth_header, sample_stock):
    response = client.get(f"/stocks/{sample_stock.id}", headers=admin_auth_header)
    assert response.status_code == 200
...
def test_create_stock(client, admin_auth_header, sample_product):
    payload = {"product_id": sample_product.id, "quantity": 50}
...
def test_update_stock_not_found(client, admin_auth_header):
    ...
    response = client.put("/stocks/99999", json=payload, headers=admin_auth_header)
    assert response.status_code == 404


def test_delete_stock(client, admin_auth_header, sample_stock):
    response = client.delete(f"/stocks/{sample_stock.id}", headers=admin_auth_header)
    assert response.status_code == 200
