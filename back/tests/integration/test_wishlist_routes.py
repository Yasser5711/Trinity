...
def test_add_to_wishlist(client, auth_header, sample_product):
    ...
    assert any(item["product"]["id"] ==
               sample_product.id for item in response.json()["items"])


def test_add_existing_product_to_wishlist_fails(client, auth_header, sample_product):
    payload = {"product_id": sample_product.id}
    client.post("/client/wishlist/items", json=payload,
                headers=auth_header)  # First add
    response = client.post("/client/wishlist/items", json=payload,
                           headers=auth_header)  # Second add
...
def test_delete_wishlist(client, auth_header, sample_product):
    client.post("/client/wishlist/items",
                json={"product_id": sample_product.id}, headers=auth_header)
