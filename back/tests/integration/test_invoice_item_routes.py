def test_create_invoice_item(client, admin_auth_header, sample_product, sample_user):
    ...
    invoice = client.post("/invoices", json=invoice_payload,
    ...
    response = client.post("/invoice-items", json=payload,
...
def test_get_invoice_item_by_id(client, admin_auth_header, sample_product, sample_user):
    ...
    matching = next(
        (i for i in all_items if i["invoice_id"] == invoice["id"]), None)

    assert matching
    response = client.get(
        f"/invoice-items/{matching['id']}", headers=admin_auth_header)
    assert response.status_code == 200
...
def test_delete_invoice_item(client, admin_auth_header, sample_product, sample_user):
    ...
    response = client.delete(
        f"/invoice-items/{item['id']}", headers=admin_auth_header)
    assert response.status_code == 200
