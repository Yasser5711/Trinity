...
def test_create_role(client, admin_auth_header):
    ...
    response = client.post("/admin/roles", json=payload, headers=admin_auth_header)
    assert response.status_code == 200
...
def test_update_role(client, admin_auth_header, sample_role):
    payload = {"id": sample_role.id, "name": "superadmin"}
    response = client.put("/admin/roles", json=payload, headers=admin_auth_header)
    assert response.status_code == 200
