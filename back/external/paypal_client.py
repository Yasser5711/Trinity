...
def get_paypal_token():
    ...
    response = requests.post(url, headers=headers, auth=auth, data=data)  # noqa: S113
...
def create_order(access_token, order_data):
    ...
    response = requests.post(url, headers=headers, json=order_data)  # noqa: S113
...
def capture_order(access_token, order_id):
    ...
    response = requests.post(url, headers=headers)  # noqa: S113
    if response.status_code == 201:
        return response.json()
