...
from db.schemas.address_schemas import AddressCreate, AddressResponse, AddressUpdate
...
@router.get(
    "/addresses/{address_id}", response_model=AddressResponse, tags=["addresses"]
)
def get_address_by_id(
    ...
    address = address_service.get_address_by_id(db, address_id, current_user.id)
    if not address:
...
@router.delete(
    "/addresses/{address_id}", status_code=status.HTTP_200_OK, tags=["addresses"]
)
def delete_address(
    address_id: int,
    current_user: User = Depends(login_required),
