...
from db.schemas.invoices_schemas import InvoiceResponse, InvoiceResponse_2
...
@router.delete(
    "/cart/items/{item_id}", status_code=status.HTTP_200_OK, tags=["client-routes"]
)
...
@router.get(
    "/invoices",
    response_model=list[InvoiceResponse_2],
    status_code=status.HTTP_200_OK,
    tags=["client-routes"],
)
def get_invoices(
...
@router.get(
    "/invoices/{invoice_id}",
    response_model=InvoiceResponse_2,
    status_code=status.HTTP_200_OK,
    tags=["client-routes"],
)
def get_invoice(
